...
function addJavaScriptInputValidation(source, fixes) {
  ...
  for (const match of matches.reverse()) {
    const parameters = match[1]
      ...
      .filter((parameter) => parameter && !parameter.includes("=") && !parameter.startsWith("..."))
    ...
    const missingParameters = parameters.filter((parameter) => {
      ...
      const existingValidation = new RegExp(
        `(?:typeof\\s+${escapedParameter}|${escapedParameter}\\s*={2,3}\\s*undefined|${escapedParameter}\\s*==?\\s*null|!${escapedParameter}\\b)`,
      );
      return !existingValidation.test(body.slice(0, 500));
    });
    if (!missingParameters.length) continue;
    const checks = missingParameters
      .map((parameter) => `  if (${parameter} === undefined) {\n    throw new TypeError('${parameter} is required');\n  }`)
      .join("\n");
...
function refactorJavaScript(code, fixes) {
  ...
  const safeDivision = source.replace(
    /\b(const|let)\s+([A-Za-z_$][\w$]*)\s*=\s*([A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)?|\d+(?:\.\d+)?)\s*\/\s*([A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)?|\d+(?:\.\d+)?)\s*;?/g,
    "$1 $2 = $4 !== 0 ? $3 / $4 : 0;",
  );
  if (safeDivision !== source) addFix(fixes, "added division-by-zero guards");
  source = safeDivision;

  const withoutDocumentWrite = source.replace(
    /\bdocument\.write\s*\([^;\n]*\);?/g,
    "// document.write() removed - security risk",
  );
  if (withoutDocumentWrite !== source) addFix(fixes, "removed document.write calls");
  source = withoutDocumentWrite;

  let flattenedConditions = source;
  let previousFlattenedSource;
  do {
    previousFlattenedSource = flattenedConditions;
    flattenedConditions = flattenedConditions.replace(
      /if\s*\(([^(){}]+)\)\s*\{\s*if\s*\(([^(){}]+)\)\s*\{([^{}]*)\}\s*\}/g,
      "if ($1 && $2) {$3}",
    );
  } while (flattenedConditions !== previousFlattenedSource);
  if (flattenedConditions !== source) addFix(fixes, "flattened nested conditions");
  source = flattenedConditions;

  const protectedCredentials = source.replace(
    /\b(const|let)\s+(password|secret|apiKey|api_key|token)\s*=\s*(?:"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'|`(?:[^`\\]|\\.)*`)\s*;?/gi,
    (match, declaration, name) => {
      const environmentName = name.replace(/([a-z])([A-Z])/g, "$1_$2").toUpperCase();
      return `${declaration} ${name} = process.env.${environmentName}; // TODO: move to environment variable`;
    },
  );
  if (protectedCredentials !== source) addFix(fixes, "moved hardcoded credentials to environment variables");
  source = protectedCredentials;

  const withoutTodos = source.replace(/^[\t ]*\/\/\s*(?:TODO|FIXME)\b.*\r?\n?/gim, "");
  if (withoutTodos !== source) addFix(fixes, "removed unresolved TODO comments");
  source = withoutTodos;
