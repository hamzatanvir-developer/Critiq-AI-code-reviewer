...
export async function analyzeRepo(files, repoMetadata) {
  try {
    if (!Array.isArray(files) || files.length === 0 || !repoMetadata?.name) {
      return null;
    }
...
Analyze these ${files.length} files from the ${repoMetadata.name} repository and return ONLY valid JSON:
{
  ...
  "improvements": array of strings (top recommendations),
  "fileReports": array of {
    "path",
    "score": number out of 100,
    "issues": number,
    "summary": string
  },
  "summary": string (2-3 sentence overall project summary)
}
